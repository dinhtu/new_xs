...
class Check6Month extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:Check6Month';
    ...
    {
        Log::channel('log_batch')->info('start batch file check 6 month');
        $maxDate = Predict::where('type', 5)->max('day');
        $startDate = empty($maxDate) ? "2021-05-01" : Carbon::parse($maxDate)->addDays(1)->format('Y-m-d');
        // $startDate = "2010-01-01";
        $now = Carbon::parse(Carbon::now()->addDays(1)->format('Y-m-d'));
        while (Carbon::parse($startDate) < $now) {
            Log::channel('log_batch')->info(Carbon::parse($startDate)->format('Y-m-d').'_check 6 month');
            ...
            foreach ($xsDetails as $xsDetail) {
                ...
                ->whereHas('xsDay', function($q) use ($startDate) {
                    ...
                    $q->whereDate('day', '>=', Carbon::parse($startDate)->addDays(-99));
                })
                ->get();
            ...
            if ($dataAll) {
                ...
                $predict->save();
                Log::channel('log_batch')->info(Carbon::parse($startDate)->format('Y-m-d'). '-complete_check 6 month');
            }
            $startDate =  Carbon::parse($startDate)->addDays(1)->format('Y-m-d');
        }
        Log::channel('log_batch')->info('complete_check 6 month');
    }
}
