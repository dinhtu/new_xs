...
class Check7 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:check7';
    ...
    {
        Log::channel('log_batch')->info('start batch file 7');
        $maxDate = Predict::where('type', 7)->max('day');
        ...
        while (Carbon::parse($startDate) < $now) {
            Log::channel('log_batch')->info(Carbon::parse($startDate)->format('Y-m-d').'-type-7');
            ...
            if ($dataAll) {
                ...
                $predict->save();
                Log::channel('log_batch')->info(Carbon::parse($startDate)->format('Y-m-d'). '-complete-type-7');
            }
